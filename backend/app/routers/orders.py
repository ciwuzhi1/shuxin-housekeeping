...
相比 Node 版修复：
- P0-2 越权读取：列表按角色强制归属；详情校验可见性
- P0-3 越权修改：状态流转按角色 + 归属校验；客户只能取消自己的订单
- P1-1 评价双写 reviews 表
- P1-2 订单完成生成 income 流水、取消（已支付）生成 refund 流水
"""
...
def review_order(order_id: str, data: OrderReviewIn, user=Depends(require_auth)):
    ...
    def _do(conn):
        execute("UPDATE orders SET rating = %s, review = %s, updated_at = %s WHERE id = %s",
